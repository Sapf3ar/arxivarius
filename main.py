...
with st.container():
    ...
    # Add unit tests here to validate the functionality above.

# Functions to be tested

def fetch_and_display_paper(paper_link: str) -> None:
    """Fetch a paper from the given Arxiv link and display its content."""
    try:
        paper_id = paper_link.split("/")[-1][:-3]
        paper = ArxivLoader(query=paper_id,
                            load_all_available_meta=False,
                            load_max_docs=1).load()[0]
        return paper
    except Exception as e:
